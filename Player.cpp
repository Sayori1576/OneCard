...
bool Player::attackplus(const Card& C)//공격 카드를 냈다면 공격하는 함수
{
...
vector<int> Player::nomal(const Card& uc)//일반적인 경우에 뽑을 수 있는 카드를 고르는 함수
{
...
vector<int> Player::cardattack(const Card& uc)//공격받았을 경우 뽑을 수 있는 카드를 고르는 함수
{
	cout << "상대방의 공격 감지" << endl;//정보 출력
	vector<int> a;//뽑을 수 있는 번호들 목록
	for (int i = 0; i < mycard.size(); i++)
	{
		if (mycard[i].kind == uc.kind || mycard[i].num == uc.num || mycard[i].kind == uc.kind||mycard[i].kind == "J")//종류나 숫자가 같거나 조커 카드인데
		{
			if (((mycard[i].type == ATTACK) && mycard[i].importance >= uc.importance) || mycard[i].type == DEFENSE)
				//방어 카드이거나 공격 카드이면서 중요도가 높다면
				a.push_back(i);
			}
